	...
	public void start() {
		if (socketHandler!=null && weightController!=null){
			...
			try {
				...
				System.out.println("Tiltænkt pbId = 2: " + pdao.getProduktBatch(2));
...
//				System.out.println("Råvarebatch kompatibel: " + rdao.getRaavare(pkdao.getProduktBatchKomp(3, 2).getRaavareBatchId()).getRaavareNavn());
				...
				//Hvis den krævede råvare er den samme råvare som i batchet 
				//og der er nok af den til at 
...
//  		weightController.showMessageSecondaryDisplay(rkdao.getReceptKompList(recdao.getRecept(pdao.getProduktBatch(pb).getReceptId()).getReceptId()).toString());
		...
		case 9:
			...
			weightController.showMessageTernaryDisplay(message.getMessage());
			try {
				//Gem reference til den pågældende produktbatch, setStatus og update
	...
	//Råvarebatch / begynd afvejning
		case 13:
			...
			try {
				...
				// Kører igennem produktbatchkomponentere
				for (int i = 0; i < pkdao.getProduktBatchKompList(pb).size(); i++) {
					...
					if (pkdao.getProduktBatchKompList(pb).get(i).getBrugerId() == 0) {
						System.out.println("Ingen bruger på produktbatchkomp " + i);
						...
						if (rkRaavare == rbRaavare && rkNomNetto < rbNomNetto) {
							System.out.println("Råvare og mængde tjek done");
	...
	case 15:
		...
		socketHandler.sendMessage(new SocketOutMessage("RM B\r\n"));
			...
			// TODO Registrer alt i databasen, SÅFREMT ALLE KRITERIER ER OPFYLDT.
			// Der skal tjekkes bruttovægt og laves rollback hvis vægten ikke er i intervallet. Implementer else return false
			// Der skal registreres slutdato
			// Der mangler også at blive registreret startdato
			// Alle attributer skal gemmes i databasen på de rigtige tidspunkter
			// Der skal tjekkes for om det producerede produktbatchkomponent er det sidste i produktbatchet
			// Status skal ændres, hvis det er
			// Laboranten skal spørges om den vil afveje endnu et komponent - rollback til trin 6, inf-2
